...
\label{e4}
\vspace{-3mm}
\min_{\mathbf{X}}\|\mathbf{W}_{1}(\mathbf{Y}-\mathbf{X})\mathbf{W}_{2}\|_{F}^{2}
+ 
\|\mathbf{X}\|_{\bm{w},*}.
\end{equation}
...
\label{e10}
\vspace{-3mm}
\min_{\mathbf{X},\mathbf{Z}}\|\mathbf{W}_{1}(\mathbf{Y}-\mathbf{X})\mathbf{W}_{2}\|_{F}^{2}
+
\|\mathbf{Z}\|_{\bm{w},*}
\quad
\text{s.t.}
...
\vspace{-3mm}
\begin{split}
\mathcal{L}(\mathbf{X},\mathbf{Z},\mathbf{\Delta},\rho)
=
&\|\mathbf{W}_{1}(\mathbf{Y}-\mathbf{X})\mathbf{W}_{2}\|_{F}^{2}
+
\|\mathbf{Z}\|_{\bm{w},*}
\\
&
+
\langle
\mathbf{\Delta},\mathbf{X}-\mathbf{Z}
\rangle
+
\frac{\rho}{2}
...
\end{split}
\end{equation}
where $\mathbf{\Delta}$ is the augmented Lagrangian multiplier and $\rho>0$ is the penalty parameter.\ We initialize the matrix variables $\mathbf{X}_{0}$, $\mathbf{Z}_{0}$, and $\mathbf{\Delta}_{0}$ to be zero matrix and $\rho_{0}>0$ to be a suitable value.\ Denote by ($\mathbf{X}_{k}, \mathbf{Z}_{k}$) and $\mathbf{\Delta}_{k}$ the optimization variables and Lagrange multiplier at iteration $k$ ($k=0,1,2,...$), respectively.\ By taking derivatives of the Lagrangian function $\mathcal{L}$ w.r.t. $\mathbf{X}$ and $\mathbf{Z}$ and setting the derivative function to be zero, we can alternatively update the variables as follows:
\vspace{2mm}
\\
(1) \textbf{Update $\mathbf{X}$ while fixing $\mathbf{Z}$ and $\mathbf{\Delta}$}:
\vspace{-2mm}
\begin{equation}
\label{e12}
\mathbf{X}_{k+1}
...
\|\mathbf{W}_{1}(\mathbf{Y}-\mathbf{X})\mathbf{W}_{2}\|_{F}^{2} 
+
\frac{\rho_{k}}{2}\|\mathbf{X} - \mathbf{Z}_{k} + \rho_{k}^{-1}\mathbf{\Delta}_{k}||_{F}^{2}.
\end{equation}
...
\mathbf{C},
\end{equation}
where $\mathbf{A}=\mathbf{W}_{1}^{\top}\mathbf{W}_{1}$, $\mathbf{B}=\frac{\rho_{k}}{2}(\mathbf{W}_{2}\mathbf{W}_{2}^{\top})^{-1}$, and $\mathbf{C}=\mathbf{W}_{1}^{\top}\mathbf{W}_{1}\mathbf{Y}+(\frac{\rho_{k}}{2}\mathbf{Z}_{k} -\frac{1}{2}\mathbf{\Delta}_{k})(\mathbf{W}_{2}\mathbf{W}_{2}^{\top})^{-1}$. This is a standard Sylvester equation which can be rewritten as 
\begin{equation}
\label{e13}
\vspace{-2mm}
(\mathbf{I}_{3p^2}\otimes\mathbf{A}
+
\mathbf{B}^{\top}\otimes\mathbf{I}_{M})\text{vec}(\mathbf{X})
=
\text{vec}(\mathbf{C}),
\end{equation}
where $\mathbf{I}_{d}$ is the $d^2$ dimensional identity matrix, $\otimes$ is the Kronecker product, and $\text{vec}(\bullet)$ is the vec-operator. Since the eigenvalues of $\mathbf{A}$ and $-\mathbf{B}$ are clearly different, the above equation has a unique solution \cite{Bartels1972,simoncini2016computational}.
\vspace{2mm}
\\
(2) \textbf{Update $\mathbf{Z}$ while fixing $\mathbf{X}$ and $\mathbf{\Delta}$}:
\vspace{-2mm}
\begin{equation}
\label{e14}
\mathbf{Z}_{k+1}
...
\|\mathbf{Z} - (\mathbf{X}_{k+1}+\rho_{k}^{-1}\mathbf{\Delta}_{k})\|_{F}^{2}
+
\|\mathbf{Z}\|_{\bm{w},*}.
\end{equation}
According to the Theorem 1 in \cite{wnnmijcv}, given the SVD of $\mathbf{X}_{k+1}+\rho_{k}^{-1}\mathbf{\Delta}_{k}$, i.e., $\mathbf{X}_{k+1}+\rho_{k}^{-1}\mathbf{\Delta}_{k}=\mathbf{U}_{k}\mathbf{\Sigma}_{k}\mathbf{V}_{k}^{\top}$, where 
$\mathbf{\Sigma}_{k}=
\left( \begin{array}{c}
...
\vspace{2mm}
 \\
(3) \textbf{Update $\mathbf{\Delta}$ while fixing $\mathbf{X}$ and $\mathbf{Z}$}:
\vspace{-3mm}
\begin{equation}
\label{e17}
\vspace{-6mm}
\mathbf{\Delta}_{k+1}
=
\mathbf{\Delta}_{k} + \rho_{k}(\mathbf{X}_{k+1}-\mathbf{Z}_{k+1}).
\end{equation}
\\
...
\\
\textbf{Initialization:} $\mathbf{X}_{0}=\mathbf{Z}_{0}=\mathbf{\Delta}_{0}=\mathbf{0}$, $\rho_{0}>0$, \text{T} = \text{False},
\\
...
(\mathbf{W}^{\top}\mathbf{W}\mathbf{Y} + \frac{\rho_{k}}{2}\mathbf{Z}_{k} -\frac{1}{2}\mathbf{\Delta}_{k})
$
\\
...
$
\\
3. Update $\mathbf{\Delta}_{k+1}$ as
$
\mathbf{\Delta}_{k+1}
=
\mathbf{\Delta}_{k} + \rho_{k}(\mathbf{X}_{k+1}-\mathbf{Z}_{k+1})
...
\begin{theorem}
\label{th1}
Assume that the weights in $\bm{w}$ are in a non-descending order, the sequences $\{\mathbf{X}_{k}\}$, $\{\mathbf{Z}_{k}\}$, and $\{\mathbf{\Delta}_{k}\}$ generated in Algorithm 1 satisfy:
\begin{align}
...
We first prove that the sequence $\{\mathbf{\Delta}_{k}\}$ generated by Algorithm 1 is upper bounded.\ Since $\{\rho_{k}\}$ is unbounded, i.e., $\lim_{k\to\infty}{\rho_{k}}=+\infty$, we can prove that the sequence of Lagrangian function $\{\mathcal{L}(\mathbf{X}_{k+1},\mathbf{Z}_{k+1},\mathbf{\Delta}_{k},\rho_{k})\}$ is also upper bounded.\ Hence, both $\{\mathbf{W}(\mathbf{Y}-\mathbf{X}_{k})\}$ and $\{\mathbf{Z}_{k}\}$ are upper bounded.\ Then $\{\mathbf{X}_{k}\}$ is also upper bounded.\ According to Eq.\ (\ref{e17}), we can prove that 
$
\lim_{k \to \infty} 
\|
\mathbf{X}_{k+1}
...
+
\rho_{k}^{-1}\|
\mathbf{\Delta}_{k}-\mathbf{\Delta}_{k-1}
\|_{F})
=
0
$
...
+
\|
\rho_{k-1}^{-1}\mathbf{\Delta}_{k-1}
+
\rho_{k}^{-1}\mathbf{\Delta}_{k+1}
-
\rho_{k}^{-1}\mathbf{\Delta}_{k}
\|_{F})
=
0,
$
where $\mathbf{U}_{k-1}\mathbf{\Sigma}_{k-1}\mathbf{V}_{k-1}^{\top}$ is the SVD of $\mathbf{X}_{k}+\rho_{k-1}^{-1}\mathbf{\Delta}_{k-1}$
.
\end{proof}
